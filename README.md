...
카드의 **제목이나 패널을 클릭**하면 해당 소스 색상이 강조된 상세 창이 열리고, 파이프라인이
미리 생성한 3구성을 보여준다 — **① 원문 번역본 ② 핵심 요약 ③ 블로그 글 작성용 초안(복사 가능)**.
("원문 보기 ↗" 링크만 원문으로 이동한다.) 정적 사이트라 이 콘텐츠는 파이프라인(LLM) 단계에서
생성해 DB(`detail_*` 컬럼)·`data.json`에 담는다. LLM 키가 없으면 원문 번역본은 요약으로 폴백하고
나머지는 "키 설정 후 생성" 안내가 뜬다.
...
│   ├── detail.mjs       # 상세 뷰 3구성 생성(원문 번역본·요약·블로그 초안)
