...
콘텐츠는 두 경로로 채워진다:
1. **파이프라인 사전 생성** — 서버(CI)에서 LLM으로 생성해 DB(`detail_*`)·`data.json`에 담음.
2. **브라우저 직접 생성(BYOK)** — 사전 생성분이 없으면, 사용자가 **우측 상단 ⚙ 설정**에서 입력한
   API 키로 브라우저가 프로바이더를 직접 호출해 생성(결과는 localStorage에 캐시).
